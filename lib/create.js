...
const logSpinner = require('./spinner')
const {
  exit,
  clearConsole,
  getTitle,
...
const { logWithSpinner, stopSpinner, changeSpinnerMsg } = logSpinner
...
async function create (projectName, options) {
  ...
  const { template } = await inquirer.prompt({
    ...
    choices: [
      ...
      { name: 'quickApp        ->  快应用工程', value: 'quick-app' },
...
async function generate (tmpDir, targetDir, params = {}) {
  ...
  if (opts.complete && typeof opts.complete === 'function') {
    try {
      await opts.complete(params, { ...utils, ...logSpinner, chalk })
    } catch (error) {
      traceError('运行 complete 异常...')
      traceError(error)
    }
  }
}
